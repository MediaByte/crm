...
export const userGroups = (state=initialState, action={}) => {
	switch(action.type) {
		...
		case SAVE_USER:
			...
			return { ...state, users: newUsers, usersCopy: newUsersCopy, selected: action.payload }
		case REMOVE_USER:
			return { ...state, users: _.filter(state.users, (user) => user.id !== action.payload), usersCopy: _.filter(state.usersCopy, (user) => user.id !== action.payload), selected: false }
		case FILTER:
