...
export default class SetGroup {
  ...
  constructor(itemSchema, gunInstance, setOnChange) {
    if (!Utils.isSchema(itemSchema)) {
      throw new TypeError('invalid schema SetGroup()')
    }
  ...
  async set(object) {
    ...
    Object.keys(Utils.getEdgeLeaves(this.itemSchema)).forEach(key => {
      ...
      const value = /** @type {WrapperGroup|null}  */ (object[key])
    ...
    return new Promise(async resolve => {
      try {
        ...
        const wrapperRef = new UserGroup(
          this.itemSchema,
          gunRef,
          false,
  ...
  get(setKey) {
    ...
    if (!(setKey in this.currentData)) {
      throw new ReferenceError(
        `unexistant key in SetGroup.get(), in set: ${
    ...
    // initial currentData for that group
    group.cachePut(this.currentData[setKey])

    this.spawnedGroups[setKey] = group
  ...
  async isValidSet(objectData) {
    ...
    Object.entries(primitiveLeaves).forEach(([key, leaf]) => {
      if (!(key in objectData)) {
        errorMap.puts(
          key,
          `missing key: ${key} in setGroup for ${
            this.itemSchema[Utils.SCHEMA_NAME]
          }, all keys of the initial value for a group (except sub-sets) must be initialized to either null, a primitive or a reference to a group`,
        )
    ...
    Object.entries(literalLeaves).forEach(([key, leaf]) => {
      ...
      if (!Utils.conformsToSchema(literalLeafType, objectData[key])) {
        errorMap.puts(
          key,
          `wrong data layout for literal given to SetGroup.set(), schema: ${JSON.stringify(
            literalLeafType,
            null,
            4,
    ...
    Object.entries(edgeLeaves).forEach(([key, leaf]) => {
      ...
      if (value !== null && !(value instanceof UserGroup)) {
        errorMap.puts(key, 'must initialize to a reference or a null value')
      }
      ...
      const conformsToSchema =
        value === null
          ...
          : Utils.conformsToSchema(
              itemSchema,
              // CAST: we already verify above that this is an instance of group
              /** @type {WrapperGroup} */ (value).currentData,
            )

      if (!value === null && !conformsToSchema) {
        errorMap.puts(key, 'wrong group type received for reference')
      }
    })
